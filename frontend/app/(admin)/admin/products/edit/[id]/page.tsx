...
const formSchema = z.object({
  ...
  productImage: z
    ...
    .min(1, "You must upload atleast one image")
...
const CreateProduct = () => {
  ...
  useEffect(() => {
    ...
    fetchProductData();
  }, [token, params.id, fetchProductData]);
  ...
  const onSubmit: SubmitHandler<ProductFormValues> = (
    ...
    data.productImage[1] &&
      formData.append("productImage", data.productImage[1]);
    data.productImage[2] &&
      formData.append("productImage", data.productImage[2]);

      axios.put(`product/v1/${params.id}`, formData,{
        headers: {
          "Content-Type": "multipart/form-data",
          authorization: `Bearer ${token}`
        ...
        setIsLoading(false);
        toast("Product updation failed try again after sometime")
      });
  };
